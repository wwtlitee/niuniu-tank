...
test("重工厂生产队列固定显示在左侧生产栏", async () => {
  ...
  assert.ok(result.order>result.minimap&&result.order<result.cmd,"生产队列应位于小地图和命令卡之间");
});
...
test("战斗音效使用按口径分层的独立合成而不是单音蜂鸣", async () => {
  const page = await openSurvival();
  const result = await page.evaluate(() => ({
    profile: sfx.profile,
  ...
  assert.deepEqual(result.profile.layers, ["pressure", "crack", "tail"]);
...
test("生存模式开局不赠送坦克且默认显示鼠标命令卡", async () => {
  ...
  assert.equal(result.commandCardVisible, true);
});
...
test("底栏在 1080p 增高并提供单选头像与多选头像矩阵", async () => {
  const page=await openSurvival();
  await page.setViewportSize({width:1920,height:1080});
  const result=await page.evaluate(() => {
    if(!player)spawnPlayer();
    wc3Select("player",player);wc3RenderSel();
    const single={height:document.getElementById("wc3dock").getBoundingClientRect().height,portrait:!!document.getElementById("spPortraitCanvas")};
  ...
  await page.close();
  assert.ok(result.height>=224&&result.height<=240,`1080p 底栏应约 232px，实际 ${result.height}`);
...
test("金矿升级只让建筑逐级温和变大且最高不越过单格视觉上限", async () => {
  const page=await openSurvival();
  const result=await page.evaluate(()=>{
  ...
  await page.close();
  assert.deepEqual(result.scales.map((value)=>+value.toFixed(2)),[1,1.05,1.1,1.15,1.2,1.25]);
...
test("标准炮台四个专精按钮显示动态金币价格", async () => {
  ...
  const result=await page.evaluate(()=>{
    ...
    return {expected,count:buttons.length,texts:buttons.map((button)=>button.textContent),disabled:buttons.map((button)=>button.getAttribute("aria-disabled")==="true")};
  });
  await page.close();
  assert.equal(result.count,4);
  for(const text of result.texts)assert.match(text,new RegExp(`💰\\s*${result.expected}`),`专精按钮必须显示价格：${text}`);
...
test("坡口一级巨岩被单只普通僵尸持续攻击时不会拖死整波", async () => {
  const page=await openSurvival();
  const result=await page.evaluate(()=>{
...
test("选中巨岩墙可显示价格并逐级升级到五十级", async () => {
  ...
  const result=await page.evaluate(()=>{
    ...
    wc3RenderSel();
    renderCmdCard();return {firstText,firstCost,afterFirst,finalLevel:wallLvAt(anchor.x,anchor.z),maxText:document.getElementById("cmdcard").textContent};
  });
  await page.close();
  assert.match(result.firstText||"",new RegExp(`Lv2.*💰\\s*${result.firstCost}`));
...
test("Boss 难度系数只应用一次", async () => {
  ...
  const result=await page.evaluate(()=>{
    ...
    const expected=Math.ceil((BOSS_TYPE.hp+(game.wave-5)*4)*(profile.hpMultiplier/4)*ACTIVE_MODE.difficultyScale(game.wave));
