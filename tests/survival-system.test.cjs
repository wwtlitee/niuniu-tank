...
test("研究院只包含防御、炮台、坦克、经济四条金币科技", () => {
  assert.deepEqual(Object.keys(RESEARCH_LINES), ["defense", "turret", "tank", "economy"]);
...
test("重工厂提供轻中重坦克与维修车且全部占用人口", () => {
  assert.deepEqual(Object.keys(FRIENDLY_UNIT_TYPES), ["light", "medium", "heavy", "repair"]);
...
test("十波战役数量递增且打完进入无尽", () => {
  ...
  assert.deepEqual(waves.map((wave) => wave.count), [10, 44, 78, 160, 280, 420, 580, 740, 880, 1000]);
  ...
  const pressure = (wave) => wave.count * wave.hpMultiplier * wave.damageMultiplier;
  ...
  assert.equal(waveProfile(11).count, Math.round(1000 * 1.1));
  ...
  assert.ok(Math.abs(waveProfile(12).count / waveProfile(11).count - 1.1) < 1e-9);
  assert.equal(bossProfile(15).id, "corrupted_colossus");
  assert.ok(waveProfile(1).activeCap > 1e6, "战役不得再设同屏上限");
  assert.ok(waveProfile(10).activeCap > 1e6, "终焉波不得再设同屏上限");
});

test("尸潮按波整批倾泻而不是几个几个出", () => {
  assert.equal(waveProfile(1).spawnBatch, 10);
  assert.equal(waveProfile(2).spawnBatch, 44);
  assert.equal(waveProfile(3).spawnBatch, 78);
  assert.equal(waveProfile(4).spawnBatch, 36);
  assert.equal(waveProfile(10).spawnBatch, 48);
  assert.equal(waveProfile(1).spawnInterval, 0);
  assert.equal(waveProfile(10).spawnInterval, 0);
  const last = waveProfile(10);
  const frames = Math.ceil(last.count / last.spawnBatch);
  assert.ok(frames <= 22, `第10波仍需 ${frames} 帧才能出完`);
});
...
test("夜景保持冷色环境与暖色功能灯的层次且限制真实光源", () => {
  ...
  assert.ok(NIGHT_VISUALS.ambientIntensity > 0.1 && NIGHT_VISUALS.ambientIntensity < 0.45);
