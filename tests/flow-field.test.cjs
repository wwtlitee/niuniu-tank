...
test('非对称通路按行进方向反推距离，不把单向下坡当成上坡通道',()=>{
 const field=new FlowField(3,1).compute([{x:2,z:0}],{passable:()=>true,canStep:(ax,az,bx)=>bx>ax});
 assert.equal(field.distanceAt(0,0),2);
 assert.deepEqual(field.directionAt(0,0),{x:1,z:0});
});

test('方向选择包含这一步的代价，不为较小邻格距离反复斜走',()=>{
 const field=new FlowField(4,3).compute([{x:3,z:1}],{passable:()=>true});
 // 两个候选：正东整条成本 2，东北成本 2.214，必须走正东。
 field.distances[field.index(1,1)]=1;field.distances[field.index(1,0)]=.8;
 assert.deepEqual(field.directionAt(0,1),{x:1,z:0});
});
