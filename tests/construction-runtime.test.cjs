...
test('在场上限保留待刷量，腾出空间按原波次刷出且不覆盖旧Boss',async()=>{
 ...
 });await page.close();assert.equal(result.capped,true);assert.equal(result.count,1200);assert.equal(result.call.wave,5);assert.equal(result.call.boss,true);assert.equal(result.pending,result.expected);
});
...
test('灼烧限时不叠伤害，双光环不重复叠加且高级维修车可读档',async()=>{
 const page=await openSurvival();const result=await page.evaluate(()=>{
 ...
 assert.deepEqual(result.aura,[1.2,1.25]);assert.deepEqual(result.left,[1,1]);assert.deepEqual(result.branches,['attack','speed','speed']);
});
...
test('炮台半尺寸且炮管俯仰与炮口方向一致',async()=>{
 const page=await openSurvival();const result=await page.evaluate(()=>{
  ...
  return {half:g.scale.x===.5,down:tur.userData.pitchPivot.rotation.x>0,aligned:forward.dot(target.clone().sub(muzzle).normalize())>.995};
 });await page.close();assert.deepEqual(result,{half:true,down:true,aligned:true});
});
