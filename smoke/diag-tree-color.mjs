/* 离屏渲染采样 5 种活体树木资产的真实颜色，判定红花簇来源 */
import { chromium } from "playwright";
const NAMES = ["tile", "tile-hill", "tile-rock", "tile-tree", "block-grass", "block-grass-low", "tile-straight-slope"];
...
const result = await p.evaluate((names) => {
  ...
  for (const n of names) {
    ...
    let rs = 0, gs = 0, bs = 0, cnt = 0, redpix = 0;
    ...
    sc2.remove(g);
    out[n] = cnt ? { avg: [Math.round(rs / cnt), Math.round(gs / cnt), Math.round(bs / cnt)], cov: +(cnt / (size * size)).toFixed(2), redFrac: +(redpix / cnt).toFixed(2) } : "blank";
  }
  r2.dispose();
  return out;
}, NAMES);
...
const verdict = (a) => {
  ...
  return `${red ? "★RED" : green ? "green" : "neutral"} rgb(${r},${g},${bb}) cov=${a.cov} redFrac=${a.redFrac}`;
};
console.log("=== 树木资产渲染平均色审计 ===");
for (const n of NAMES) console.log(`  ${n.padEnd(16)} ${verdict(result[n])}`);
process.exit(0);
