  ...
  if(ACTIVE_MODE.mapType==="survival"){
    ...
    function decorateSurvivalCliff(){
      ...
      for(let z=E.z0-1;z<=E.z1+1;z++)for(let x=E.x0-1;x<=E.x1+1;x++){
        ...
        if(Math.random()<.55){
          const c=cellCenter(x,z);
          /* ★P5c-R6：rocks 数组原为 TD kit 的 detail-rocks/rocks-small（橙红岩），
             是贴墙红簇来源——统一改 platformer 同族灰绿小物 */
    ...
    for(let z=0;z<GRID;z++)for(let x=0;x<GRID;x++){
      ...
      switch(t){
        ...
        case T_TREE:{
          ...
          const names=["tree_default","tree_oak","tree_cone","tree_detailed","tree_fat"];
          placeModel(mapGroup,names[(Math.random()*names.length)|0],
            c.x,c.z,TILE*.92,Math.random()*6.28,0,0x2e5d2e,3.4,6.5);
          break;}
        case T_ROAD:{
...
/* ★ P4-2：重建单格坡道板（tile-slant）。场景：坡道格建墙→墙破/拆后恢复通行。
   逻辑与 buildMapMeshes 的 T_RAMP 分支一致（缩放/旋转/贴 base）。 */
