...
const SURVIVAL_ACTIVE_LIMIT=1200;
function enqueueSurvivalWave(wave,preservePending){
  if(!preservePending)game.spawnPlans=[];
  if(!Array.isArray(game.spawnPlans))game.spawnPlans=[];
  game.spawnPlans.push({wave,remaining:SurvivalSystem.waveProfile(wave).count,bossSpawned:false});
