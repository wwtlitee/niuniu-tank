/* 装甲竞速 v10.1.0 — 房主权威规则，PeerJS 仅负责浏览器间连接。 */
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./racing-rules.js'));else root.RacingNetwork=factory(root.RacingRules);})(typeof window==='object'?window:globalThis,function factory(R){
 'use strict';
 const PROTOCOL=5,PREFIX='niuniu-race-1-',MAX=6;
 const FIELDS=['x','y','z','heading','speed','progress','lastS','safe','lap','slot','shield','boost','stun','cooldown','offroad','aim','pitch','invulnerable','steering','slip','driftCharge','finishTime','lift','threatGuard','color','body','supplyPass','supplyCount'];
 ...
 function pack(g){return {trackId:g.trackId,trackRevision:g.trackRevision,phase:g.phase,time:round(g.time),countdown:round(g.countdown),humans:g.humans,
  // 冲线时刻和漂移蓄力保留精度，量化不能反转名次或提前跨越奖励阈值。
  cars:g.cars.map(c=>[...FIELDS.map(k=>k==='finishTime'||k==='driftCharge'?c[k]:round(c[k])),c.finished,c.drifting,c.inventory[0],c.inventory[1],c.name]),
 ...
 function unpack(p){
  if(!p||p.trackId!==R.track.id||p.trackRevision!==R.track.revision)return null;
  if(!['countdown','racing','paused','finished'].includes(p.phase)||!Number.isFinite(p.time)||p.time<0||p.time>481||!Number.isFinite(p.countdown)||p.countdown<0||p.countdown>3)return null;
  ...
  if(g.cars.some(c=>R.bodyOf(c.body)!==c.body||!Number.isInteger(c.supplyPass)||!Number.isInteger(c.supplyCount)||c.supplyCount<0||c.supplyCount>2))return null;
 ...
 class Session{
  constructor({Peer,peerOptions={},now=()=>performance.now(),onChange=()=>{},onStart=()=>{},onSnapshot=()=>{},onExit=()=>{},onTrack=()=>{}}={}){
   Object.assign(this,{Peer,peerOptions,now,onChange,onStart,onSnapshot,onExit,onTrack,role:'off',slot:0,code:'',members:Array(MAX).fill(null),links:new Map(),game:null,match:0,status:'',seq:0,actionSeq:0,rtt:0,epoch:0});
  }
  ...
  announce(){this.broadcast({type:'room',members:this.members,code:this.code,trackId:R.track.id,trackRevision:R.track.revision});this.changed();}
  connectPeer(id){
   ...
   peer.on('open',()=>{if(epoch!==this.epoch)return;this.lastSeen=this.now();if(this.role==='host'){this.opened=true;this.changed('房间已创建，邀请朋友加入');}
    else{const c=peer.connect(PREFIX+this.code,{reliable:true,serialization:'json'});this.connection=c;c.on('open',()=>this.send(c,{type:'hello',protocol:PROTOCOL,name:this.name,color:this.color,body:this.body}));c.on('data',m=>{if(epoch===this.epoch)this.receive(m);});c.on('close',()=>{if(epoch===this.epoch&&this.role==='client')this.leave('与房主的连接已断开');});c.on('error',()=>{if(epoch===this.epoch)this.leave('无法连接房主，请检查双方网络或稍后重试');});}});
  }
  setup(role,name,code,color=0,body=0){this.leave();this.role=role;this.name=nameOf(name);this.color=colorOf(color);this.body=R.bodyOf(body);this.code=code;this.slot=0;this.members=Array(MAX).fill(null);this.lastSeen=this.now();this.lastPing=0;this.lastSend=0;this.lastSnapshot=0;this.seq=0;this.actionSeq=0;this.attempts=0;this.match=0;this.opened=false;this.changed('正在连接房间服务…');
   if(!this.Peer){this.leave('联机文件未加载，请刷新页面');return false;}return true;}
  host(name,color=0,body=0){if(!this.setup('host',name,roomCode(),color,body))return;this.members[0]={name:this.name,color:this.color,body:this.body,ready:true};try{this.connectPeer(PREFIX+this.code);}catch{this.leave('浏览器暂不支持联机连接');}}
  join(code,name,color=0,body=0){code=codeOf(code);if(!/^[A-HJ-NP-Z2-9]{6}$/.test(code)){this.changed('请输入六位房间码');return;}if(!this.setup('client',name,code,color,body))return;try{this.connectPeer();}catch{this.leave('浏览器暂不支持联机连接');}}
  accept(c){
   ...
   c.on('data',m=>{if(!this.links.has(c)||!m||typeof m!=='object')return;const now=this.now();if(now-entry.window>=1000){entry.window=now;entry.budget=0;}if(++entry.budget>100){c.close();return;}entry.last=now;
    if(m.type==='hello'&&entry.slot===null){let reason='';if(m.protocol!==PROTOCOL)reason='游戏版本不同，请双方刷新页面';else if(this.game)reason='比赛已开始，请等待下一场';const slot=this.members.findIndex((v,i)=>i>0&&!v);if(slot<0)reason='房间已满（最多六人）';
     if(reason){this.send(c,{type:'reject',reason});setTimeout(()=>c.close(),100);return;}entry.slot=slot;this.members[slot]={name:nameOf(m.name),color:colorOf(m.color),body:R.bodyOf(m.body),ready:false};this.send(c,{type:'welcome',protocol:PROTOCOL,slot,code:this.code,members:this.members,trackId:R.track.id,trackRevision:R.track.revision});this.announce();return;}
    if(entry.slot===null)return;
    if(m.type==='ping'){this.send(c,{type:'pong',time:m.time});return;}
    if(m.type==='profile'&&!this.game){this.members[entry.slot]={name:nameOf(m.name),color:colorOf(m.color),body:R.bodyOf(m.body),ready:false};this.announce();return;}
  ...
  validMembers(m){return Array.isArray(m)&&m.length===6&&m.every(v=>v===null||v&&typeof v.name==='string'&&typeof v.ready==='boolean'&&Number.isInteger(v.color)&&v.color>=0&&v.color<R.PALETTE.length&&Number.isInteger(v.body)&&R.bodyOf(v.body)===v.body);}
  receive(m){
   ...
   if(m.type==='welcome'){if(m.protocol!==PROTOCOL||!Number.isInteger(m.slot)||m.slot<1||m.slot>5||!this.validMembers(m.members)){this.leave('房间数据不兼容，请刷新');return;}if(m.trackId!==R.track.id){try{R.forTrack(m.trackId);}catch{this.leave('房主赛道不可用，请刷新');return;}this.onTrack(m.trackId,this.code);this.leave('正在切换至房主赛道');return;}if(m.trackRevision!==R.track.revision){this.leave('赛道版本不同，请双方刷新');return;}this.slot=m.slot;this.members=m.members;this.opened=true;this.changed('已加入房间，准备后等待房主发车');return;}
  ...
  profile(name,color,body=this.body){if(this.game||!this.opened)return;this.name=nameOf(name);this.color=colorOf(color);this.body=R.bodyOf(body);if(this.role==='host'){this.members[0]={name:this.name,color:this.color,body:this.body,ready:true};this.announce();}else if(this.role==='client')this.send(this.connection,{type:'profile',name:this.name,color:this.color,body:this.body});}
  ...
  start(){if(this.role!=='host'||!this.opened||this.game||this.members.some(m=>m&&!m.ready))return false;
   this.game=R.create({humans:this.members.flatMap((m,i)=>m?[i]:[])});this.members.forEach((m,i)=>{if(m){this.game.cars[i].name=m.name;this.game.cars[i].color=m.color;this.game.cars[i].body=m.body;}});this.match++;this.snapshotSeq=0;
 ...
 return {Session,pack,unpack,cleanInput,PROTOCOL,forTrack:id=>factory(R.forTrack(id))};
});
