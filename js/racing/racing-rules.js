/* 装甲竞速 v8.2.0 — 纯规则模块，浏览器与 Node 共用。 */
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.RacingRules=api;})(typeof window==='object'?window:globalThis,function(){
 'use strict';
 ...
 const anchors=[[0,0],[0,140],[65,210],[155,175],[190,65],[270,90],[325,20],[290,-105],[160,-160],[45,-130],[-45,-65]];
 ...
 for(let i=0;i<anchors.length;i++)for(let j=0;j<48;j++){
  ...
  points.push({x:cat(a[0],b[0],c[0],d[0],t),z:cat(a[1],b[1],c[1],d[1],t),s:0});
 }
 ...
 function height(s){return 1.2+5*Math.pow(Math.sin(wrap(s,length)/length*Math.PI*2),4);}
 const shortStart=points[48*3].s,shortEnd=points[48*5].s,sa=sample(shortStart),sb=sample(shortEnd);
 const shortcut={start:shortStart,end:shortEnd,a:sa,b:sb,length:Math.hypot(sb.x-sa.x,sb.z-sa.z),width:9};
 const track={points,length,width:24,shortcut};
 ...
 function locate(x,z){
  ...
  for(let i=0;i<points.length-1;i++){
   ...
   if(dist<best.distance)best={...q,distance:dist,s:a.s+(b.s-a.s)*q.t,width:track.width,shortcut:false};
  }
  const q=segment(x,z,sa,sb),distance=Math.hypot(x-q.x,z-q.z);
  // 交汇区按最近中心线归属，保持捷径端点进度连续。
  if(distance<shortcut.width/2&&distance<best.distance)best={...q,distance,s:shortStart+(shortEnd-shortStart)*q.t,width:shortcut.width,shortcut:true};
  return best;
 }
 ...
 function create(options={}){
  ...
  return {multiplayer:Array.isArray(options.humans),humans:Array.isArray(options.humans)?[...new Set(options.humans.filter(id=>Number.isInteger(id)&&id>=0&&id<6))]:[0],phase:'countdown',beforePause:null,countdown:3,time:0,cars,shots:[],mines:[],threats:[],events:[],results:[],seed:1234567,
 ...
 function recover(g,c){
  ...
  const p=sample(c.safe);Object.assign(c,{x:p.x,z:p.z,y:p.y,heading:p.heading,speed:0,offroad:0,lastS:p.s,progress:c.safe,lap:clamp(Math.floor(Math.max(0,c.safe)/length),0,3),invulnerable:2,slip:0,steering:0,drifting:false,driftCharge:0});
 ...
 function updateThreats(g,dt){
  for(const t of g.threats){const c=g.cars[t.target];if(!c||c.finished){t.done=true;continue;}t.age+=dt;
   ...
   if(t.grabbed){const f=Math.min(1,(t.age-1.2)/1.8),progress=Math.max(-10,t.origin-40*f*f*(3-2*f)),p=sample(progress);Object.assign(c,{x:p.x+(t.laneX||0)*(1-f),z:p.z+(t.laneZ||0)*(1-f),y:p.y,heading:p.heading,progress,safe:progress,lastS:p.s,lap:clamp(Math.floor(Math.max(0,progress)/length),0,3)});c.lift=Math.max(.01,6*Math.sin(f*Math.PI));
 ...
 function move(g,c,dt,input){
  ...
  c.drifting=!!input.drift&&c.speed>13&&Math.abs(c.steering)>.2&&c.offroad===0;
  ...
  if(c.drifting&&Math.abs(c.slip)>.12)c.driftCharge=clamp(c.driftCharge+dt*Math.abs(c.slip)*1.4,0,1);
  if(wasDrifting&&!c.drifting){if(c.driftCharge>=.6&&c.offroad===0&&c.speed>13){c.boost=Math.max(c.boost,.85);c.speed=Math.min(58,c.speed+4);event(g,'driftboost',c);if(c.id===0)say(g,'漂移出弯 · 推进！');}c.driftCharge=0;}
  const travelHeading=c.heading-c.slip;
  c.x+=Math.sin(travelHeading)*c.speed*dt;c.z+=Math.cos(travelHeading)*c.speed*dt;
  const q=locate(c.x,c.z);c.y=height(q.s);
  let delta=angle((q.s-c.lastS)/length*TAU)/TAU*length;
  // 只接受连续行驶产生的路线变化，禁止跳点刷圈。
  if(Math.abs(delta)<Math.abs(c.speed)*dt*5+3&&q.distance<q.width/2+4){
  ...
  if(q.distance>q.width/2){
   c.speed*=Math.exp(-1.8*dt);c.offroad+=dt;
   if(q.distance>q.width/2+10||c.offroad>2)recover(g,c);
  }else c.offroad=0;
  ...
  if(c.progress>=length*3&&!c.finished){c.finished=true;c.finishTime=g.time;g.results.push(c.id);event(g,'finish',c);if(!g.multiplayer&&c.id===0)g.phase='finished';}
 }
 function step(g,dt,input={}){
  ...
  for(const c of g.cars){if(c.finished)continue;const human=g.humans.includes(c.id),control=human?(g.multiplayer?input.players?.[c.id]||{}:input):autopilot(c,g);move(g,c,dt,control);
   ...
   if(!human){c.aiUse-=dt;c.aiFire-=dt;if(c.aiUse<=0){if(!c.item)switchItem(g,c);useItem(g,c);c.aiUse=2+random(g)*3;}const target=g.cars.find(o=>o!==c&&!o.finished&&Math.hypot(o.x-c.x,o.z-c.z)<65&&Math.abs(angle(Math.atan2(o.x-c.x,o.z-c.z)-c.heading))<.12);if(target&&c.aiFire<=0){c.pitch=Math.atan2(target.y-c.y,Math.hypot(target.x-c.x,target.z-c.z));if(fire(g,c))c.aiFire=3+random(g)*2;}}
  }
  for(let i=0;i<g.cars.length;i++)for(let j=i+1;j<g.cars.length;j++){
   const a=g.cars[i],b=g.cars[j],dx=b.x-a.x,dz=b.z-a.z,d=Math.hypot(dx,dz);if(d>=3.6||d<.001||a.finished||b.finished||a.lift>0||b.lift>0)continue;
  ...
  for(const m of g.mines){m.ttl-=dt;m.arm-=dt;for(const c of g.cars)if(m.arm<=0&&c.id!==m.owner&&Math.hypot(c.x-m.x,c.z-m.z)<3.2){hit(g,c);m.ttl=0;break;}}
 ...
 return {create,step,rollItem,ITEMS,ITEM_NAMES,PALETTE,predictCar,pause,track,sample,supplyPosition,locate,height,autopilot,hit,fire,useItem,switchItem,recover,ranking,LIMITS,angle};
});
