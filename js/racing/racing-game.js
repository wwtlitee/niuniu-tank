...
(async()=>{
 ...
 const sand=mat(isCity?0x64747b:0xa79572),roadMat=mat(0x555851),edgeMat=mat(0x9c8b64);
 ...
 const skyContext=skyCanvas.getContext('2d'),skyGradient=skyContext.createLinearGradient(0,0,0,256);skyGradient.addColorStop(0,isCity?'#427792':'#497b9a');skyGradient.addColorStop(.55,'#9eafb5');skyGradient.addColorStop(1,'#d0c3a8');skyContext.fillStyle=skyGradient;skyContext.fillRect(0,0,2,256);
 ...
 function ribbon(samples,width,material,lift=0){
  ...
  samples.forEach((p,i)=>{
   ...
   const span=typeof width==='function'?width(p):width;for(const side of [-1,1]){vertices.push(p.x+dz/l*span*.5*side,p.y+lift,p.z-dx/l*span*.5*side);uvs.push(side<0?0:1,i/8);}
 ...
 const runoffMat=mat(isCity?0x899394:0x9c8b64,{roughness:1});
 ribbon(samples,p=>R.track.width+(isCity&&p.y>8?4:24),runoffMat,-.18);
 if(!isCity)ribbon(samples,R.track.width,roadMat);
 ...
 const trackDetails=isCity?{}:RacingTrack.create(scene);
 ...
 for(let s=0;s<(isCity?0:R.track.length);s+=7){
  ...
  for(const side of [-1,1]){
   ...
   if(Math.abs(s-shortcut.start)>22&&Math.abs(s-shortcut.end)>22){
    railRecords.push({x:p.x+cos*26.3*side,y:p.y+.6,z:p.z-sin*26.3*side,ry:p.heading,sx:.55,sy:1.15,sz:6.7,color:Math.floor(s/7)%3===0?0xb58c4b:0x777969});
    if(Math.floor(s/7)%2===0)posts.push({x:p.x+cos*27.2*side,y:p.y+1.5,z:p.z-sin*27.2*side,ry:p.heading,sx:.16,sy:3,sz:.16});
   }
  }
 }
 for(const list of [railRecords,posts])for(let i=list.length-1;i>=0;i--)if(!RacingTrack.clearsShortcut(list[i].x,list[i].z,Math.hypot(list[i].sx,list[i].sz)/2+.6))list.splice(i,1);
 ...
 for(let i=0;i<(isCity?0:230);i++){const p=R.sample(rand()*R.track.length),side=rand()>.5?1:-1,d=27+rand()*6;pebbles.push({x:p.x+Math.cos(p.heading)*d*side,y:p.y-.4,z:p.z-Math.sin(p.heading)*d*side,sx:1+rand()*2,sy:.5+rand()*1.4,sz:1+rand(),ry:rand()*6});}
 for(let i=pebbles.length-1;i>=0;i--)if(!RacingTrack.clearsShortcut(pebbles[i].x,pebbles[i].z,Math.max(pebbles[i].sx,pebbles[i].sz)+.6))pebbles.splice(i,1);
 for(const rock of rocks){const q=R.locate(rock.x,rock.z),factor=Math.min(1,Math.max(.1,q.distance-q.width/2-14)/Math.hypot(rock.sx,rock.sz));rock.sx*=factor;rock.sz*=factor;}
 ...
 try{spectacles=await (isCity?RacingCity.create(scene):RacingSpectacle.create(scene));if(isCity)Object.assign(trackDetails,spectacles.stats);}catch(e){fail(e.message||'赛道模型加载失败，请刷新重试');return;}
 ...
 const banner=sign((isCity?'SKYLINE':'CANYON')+'  /  START — FINISH',25,3);banner.position.set(0,7.6,-.5);banner.rotation.y=Math.PI;gate.add(banner);
 ...
 for(const [s,label] of [[shortcut.start-15,'SHORTCUT  ↗'],[R.track.length*.18,'BRAKE  /  弯道'],[R.track.length*.57,isCity?'SKYWAY  /  高架':'CANYON  SECTOR 02']]){
 ...
 for(let i=0;i<(isCity?0:4);i++){const p=R.sample(-35-i*18),x=p.x-29,y=p.y,z=p.z;
 ...
 const cameraBlockers=scene.children.filter(o=>o!==sky&&o!==ground&&(o.isMesh||o.isGroup));
 const cameraRay=new T.Raycaster(),cameraAnchor=new T.Vector3(),cameraDirection=new T.Vector3();
 const vehicleMaterial=mat(0xffffff,{vertexColors:true});
 function geometryFor(color,body){return body?RacingLegacyModels.tank(color,body):M.tank(color);}
 function makeCockpit(color,body=0){const source=geometryFor(color,body),view=M.cockpitFromTank(source);source.body.dispose();source.turret.dispose();return view;}
 function makeTank(color){
  const group=new T.Group(),geometry=M.tank(color),material=vehicleMaterial.clone(),body=new T.Mesh(geometry.body,material);group.add(body);
  body.castShadow=true;body.receiveShadow=true;const turret=new T.Group();turret.position.y=1.65;turret.add(new T.Mesh(geometry.turret,material));turret.children[0].castShadow=true;turret.children[0].receiveShadow=true;group.add(turret);
  const shield=new T.Mesh(new T.SphereGeometry(3.7,20,12),new T.MeshBasicMaterial({color:0x87dedd,transparent:true,opacity:.15,wireframe:true}));shield.position.y=1.2;shield.visible=false;group.add(shield);scene.add(group);return {group,turret,shield,body,material,colorIndex:-1};
 }
 ...
 const tanks=colors.map(makeTank),cockpitGeometry=makeCockpit(colors[0]);
 const cockpit=new T.Group();camera.add(cockpit);cockpit.add(new T.Mesh(cockpitGeometry.body,vehicleMaterial));
 const cockpitGun=new T.Group();cockpit.add(cockpitGun);cockpitGun.add(new T.Mesh(cockpitGeometry.gun,vehicleMaterial));cockpitGun.position.set(0,cockpitGeometry.pivot.y,cockpitGeometry.pivot.z);
 ...
 // Allocate instance colors before the first shader compile so tier colors render on Three r128.
 const effects=instances(new T.IcosahedronGeometry(.6,0),new T.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:.8}),Array(128).fill({x:0,y:-100,z:0,color:0xffffff}));
 const sparkColors=[linear('#ffc474'),...R.DRIFT_TIERS.map(t=>linear(t.color))];
 let driftRewardUntil=0,driftRewardTier=0;
 ...
 let playerSlot=0,localMenu=false,chosenColor=0,chosenBody=0,liftingCamera=false;
 let cameraMode='third',cameraPose=null,cameraLastAt=performance.now(),cameraReset=true,cameraClearance=0;
 try{if(localStorage.getItem('tank-racing-view')==='first')cameraMode='first';}catch{}
 function setView(mode){cameraMode=mode==='first'?'first':'third';cameraPose=null;cameraReset=true;$('viewMode').value=cameraMode;$('cameraButton').textContent=(cameraMode==='third'?'第三人称':'第一人称')+' · V';document.body.classList.toggle('thirdPerson',cameraMode==='third');try{localStorage.setItem('tank-racing-view',cameraMode);}catch{}}
 $('cameraButton').onclick=()=>{setView(cameraMode==='third'?'first':'third');document.activeElement?.blur();};$('viewMode').onchange=e=>setView(e.target.value);setView(cameraMode);
 const keys=new Set(),mapDots=[];
 const aimPoint=new T.Vector3(),remotePoint=new T.Vector3(),labelPoint=new T.Vector3();
  const isMobile=matchMedia('(pointer:coarse)').matches;
  const QUALITY={high:{ratio:1.5,shadows:true},medium:{ratio:1.0,shadows:true},low:{ratio:1.0,shadows:false}},NEXT_TIER={high:'medium',medium:'low'};
  function readSettings(){try{const s=JSON.parse(localStorage.getItem('tank-racing-settings')||'{}');if(['auto','high','medium','low'].includes(s.quality))quality=s.quality;if(typeof s.shake==='boolean')shake=s.shake;if(typeof s.sound==='boolean')sound=s.sound;}catch{}}
  ...
  function resize(){const max=isMobile?1:QUALITY[effectiveQuality].ratio;const ratio=Math.min(devicePixelRatio||1,max);renderer.setPixelRatio(ratio);renderer.setSize(innerWidth,innerHeight);camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();}
  function setQuality(value){if(!QUALITY[value])return;effectiveQuality=value;renderer.shadowMap.enabled=QUALITY[value].shadows;resize();}
  setQuality(quality==='low'?'low':quality==='medium'?'medium':'high');addEventListener('resize',resize);
 ...
 $('circuitEyebrow').textContent=R.track.english+' / '+R.track.name;$('circuitHud').textContent=R.track.name;$('roomCircuit').textContent=R.track.name+' · '+R.track.description;
 for(const circuit of RacingCircuits.list()){const a=document.createElement('a'),url=new URL(location.href);url.searchParams.set('track',circuit.id);url.searchParams.delete('room');a.href=url.href;a.dataset.track=circuit.id;a.setAttribute('aria-current',circuit.id===R.track.id?'page':'false');const name=document.createElement('b'),detail=document.createElement('small');name.textContent=circuit.name;detail.textContent=circuit.description;a.append(name,detail);$('circuitPicker').appendChild(a);}
 if(isCity){const xs=R.track.points.map(p=>p.x),zs=R.track.points.map(p=>-p.z),minX=Math.min(...xs)-30,minZ=Math.min(...zs)-30;$('map').setAttribute('viewBox',[minX,minZ,Math.max(...xs)-minX+30,Math.max(...zs)-minZ+30].join(' '));}
 ...
 function shapeTank(tank,body){
  body=R.bodyOf(body);if(tank.bodyIndex===body)return;
  const geometry=geometryFor(colors[0],body);
  tank.body.geometry.dispose();tank.turret.children[0].geometry.dispose();
  tank.body.geometry=geometry.body;tank.turret.children[0].geometry=geometry.turret;
  tank.turret.position.y=geometry.turretHeight||1.65;tank.bodyIndex=body;tank.colorIndex=-1;
 }
 let cockpitColor=-1,cockpitBody=-1;
 try{const profile=JSON.parse(localStorage.getItem('tank-racing-profile')||'{}');if(typeof profile.name==='string')$('playerName').value=profile.name.slice(0,16);if(Number.isInteger(profile.color)&&colors[profile.color])chosenColor=profile.color;chosenBody=R.bodyOf(profile.body);}catch{}
 function saveProfile(){try{localStorage.setItem('tank-racing-profile',JSON.stringify({name:$('playerName').value.trim().slice(0,16)||'车手',color:chosenColor,body:chosenBody}));}catch{}}
 function colorSelection(){for(const id of ['menuColors','roomColors'])for(const button of $(id).children)button.setAttribute('aria-pressed',String(Number(button.dataset.color)===chosenColor));$('previewName').textContent=R.BODIES[chosenBody].name;$('previewBody').textContent=R.BODIES[chosenBody].origin+' / '+R.BODIES[chosenBody].detail;for(const id of ['menuBodies','roomBodies'])for(const button of $(id).children)button.setAttribute('aria-pressed',String(Number(button.dataset.body)===chosenBody));}
 for(const id of ['menuColors','roomColors'])colors.forEach((color,index)=>{const b=document.createElement('button');b.type='button';b.dataset.color=index;b.style.setProperty('--swatch',color);b.setAttribute('aria-label',['赤焰红','沙漠金','翡翠绿','冰川蓝','星际紫','钛白'][index]);b.onclick=()=>{chosenColor=index;colorSelection();saveProfile();if(net.role!=='off')net.profile($('roomName').value,chosenColor,chosenBody);};$(id).appendChild(b);});
 for(const id of ['menuBodies','roomBodies'])for(const entry of R.BODIES){
  const button=document.createElement('button');button.type='button';button.dataset.body=entry.id;
  const name=document.createElement('b'),origin=document.createElement('small');name.textContent=entry.name;origin.textContent=entry.origin;
  button.append(name,origin);button.onclick=()=>{chosenBody=entry.id;colorSelection();saveProfile();if(net.role!=='off')net.profile($('roomName').value,chosenColor,chosenBody);};$(id).appendChild(button);
 }
 colorSelection();$('playerName').onchange=()=>{colorSelection();saveProfile();};$('roomName').onchange=()=>{const name=$('roomName').value.trim().slice(0,16)||'车手';$('playerName').value=name;saveProfile();net.profile(name,chosenColor,chosenBody);};
 ...
 const net=new RacingNetwork.Session({Peer:window.Peer,peerOptions:window.RACING_NETWORK_CONFIG?.peerOptions||{},onChange:roomUI,onTrack(id,code){const url=new URL(location.href);url.searchParams.set('track',id);url.searchParams.set('room',code);location.assign(url.href);},
  onStart(g,slot){game=g;playerSlot=slot;started=true;cameraReset=true;localMenu=false;manual=false;lastSnapshotAt=performance.now();accumulator=0;last=performance.now();keys.clear();lastEvent=0;skidMarks.length=0;lastSkidTime=0;driftRewardUntil=0;driftRewardTier=0;document.activeElement?.blur();initAudio();$('lobby').hidden=true;$('networkRoom').hidden=true;$('hud').hidden=false;$('pause').hidden=true;$('results').hidden=true;mapDots.forEach((d,i)=>{d.setAttribute('r',i===slot?7:5);d.setAttribute('fill',i===slot?'#fff':'#d89c66');});},
 ...
 $('createRoom').onclick=()=>{initAudio();net.host($('playerName').value,chosenColor,chosenBody);};$('joinRoom').onclick=()=>{initAudio();net.join($('roomInput').value,$('playerName').value,chosenColor,chosenBody);};$('roomStart').onclick=()=>net.start();$('readyRoom').onclick=()=>net.ready(!net.members[net.slot]?.ready);
 for(const id of ['leaveRoom','leaveRace','leaveResults'])$(id).onclick=()=>net.leave('已退出房间');
 $('copyRoom').onclick=async()=>{const url=new URL(location.href);url.search='';url.searchParams.set('track',R.track.id);url.hash='room='+net.code;try{await navigator.clipboard.writeText(url.href);$('roomStatus').textContent='邀请链接已复制';}catch{$('roomStatus').textContent='房间码：'+net.code+'，请发给朋友';}};
 ...
 function startRace(){
  if(net.role!=='off')return;playerSlot=0;$('netStatus').textContent='';mapDots.forEach((d,i)=>{d.setAttribute('r',i===0?7:5);d.setAttribute('fill',i===0?'#fff':'#d89c66');});document.activeElement?.blur();initAudio();game=R.create();game.cars[0].name=$('playerName').value.trim().slice(0,16)||'车手';game.cars[0].color=chosenColor;game.cars[0].body=chosenBody;started=true;cameraReset=true;accumulator=0;keys.clear();lastEvent=0;skidMarks.length=0;lastSkidTime=0;driftRewardUntil=0;$('lobby').hidden=true;$('hud').hidden=false;$('pause').hidden=true;$('results').hidden=true;render();
 }
 ...
 $('start').onclick=startRace;$('again').onclick=()=>{if(net.role==='host')net.returnLobby();else if(net.role==='off')startRace();};$('restart').onclick=startRace;$('pauseButton').onclick=togglePause;$('resume').onclick=togglePause;
   $('quality').onchange=e=>{quality=e.target.value;setQuality(quality==='low'?'low':quality==='medium'?'medium':'high');saveSettings();};$('shake').onchange=e=>{shake=e.target.checked;saveSettings();};$('sound').onchange=e=>{sound=e.target.checked;if(sound)initAudio();saveSettings();};
 addEventListener('keydown',e=>{
  ...
  if(e.repeat)return;
  if(e.code==='KeyV'){setView(cameraMode==='third'?'first':'third');return;}
 ...
 function showResults(){
  ...
  function hud(){
   const c=game.cars[playerSlot],rank=frameRank.findIndex(o=>o.id===playerSlot)+1;$('position').innerHTML=`${rank}<span>/ 6</span>`;$('lap').textContent=`第 ${Math.min(3,c.lap+1)} / 3 圈`;$('timer').textContent=timeText(game.time);$('speed').textContent=Math.round(Math.abs(c.speed)*3.6);$('speedFill').style.width=`${Math.min(100,Math.abs(c.speed)/58*100)}%`;
  ...
  const tier=R.driftTier(c.driftCharge),tierInfo=R.DRIFT_TIERS[Math.max(0,tier-1)];$('driftFill').style.width=`${c.driftCharge*100}%`;$('driftMeter').style.setProperty('--drift-color',tier?tierInfo.color:'#a5b8b4');$('driftMeter').dataset.tier=tier;$('driftLabel').textContent=c.drifting?(tier?tierInfo.name+'蓄力 · 松开 Shift 推进':'漂移中 · 持续压弯'):c.boost>0?((driftRewardUntil>game.time?R.DRIFT_TIERS[Math.max(0,driftRewardTier-1)].name+'出弯':'氮气')+'推进 · '+c.boost.toFixed(1)+'s'):'Shift + 转向 · 漂移蓄力';$('aimReadout').textContent=`炮口 ${Math.round(c.aim*180/Math.PI)}° / ${Math.round(c.pitch*180/Math.PI)}°`;
  ...
  let renderTime=0,frameRank=null;
  function render(){
   renderTime=started?game.time:performance.now()/1000;
   frameRank=R.ranking(game);
  const previewSlot=net.role!=='off'&&net.opened?net.slot:0;if(!started){for(const c of game.cars){c.color=net.members[c.id]?.color??c.id;c.body=net.members[c.id]?.body??c.id;}game.cars[previewSlot].color=chosenColor;game.cars[previewSlot].body=chosenBody;const park=R.sample(-65);Object.assign(game.cars[previewSlot],{x:park.x,y:park.y,z:park.z,heading:park.heading});}
  for(const c of game.cars){const m=tanks[c.id];shapeTank(m,c.body);paintTank(m,c.color);if(net.role==='client'&&started&&Math.hypot(m.group.position.x-c.x,m.group.position.z-c.z)<12){m.group.position.lerp(remotePoint.set(c.x,c.y+c.lift,c.z),.35);m.group.rotation.y+=R.angle(c.heading-m.group.rotation.y)*.35;}else{m.group.position.set(c.x,c.y+c.lift,c.z);m.group.rotation.y=c.heading;}m.group.rotation.z=-c.slip*.045;m.group.visible=started?(c.id!==playerSlot||cameraMode==='third'):c.id===previewSlot;m.turret.rotation.y=c.aim;m.turret.rotation.x=-c.pitch;m.shield.visible=c.shield>0;}
  if(started){
   const c=game.cars[playerSlot],now=performance.now(),dt=Math.min(.1,Math.max(1/240,(now-cameraLastAt)/1000));cameraLastAt=now;
   const pose=RacingCamera.desired(c,cameraMode);
   if(cameraPose&&(Math.hypot(pose.eye.x-cameraPose.eye.x,pose.eye.z-cameraPose.eye.z)>35||Math.abs(pose.eye.y-cameraPose.eye.y)>6))cameraReset=true;
   cameraPose=cameraMode==='first'?pose:RacingCamera.smooth(cameraReset?null:cameraPose,pose,dt);cameraReset=false;
   camera.position.set(cameraPose.eye.x,cameraPose.eye.y,cameraPose.eye.z);cameraClearance=0;
   if(cameraMode==='third'){
    cameraAnchor.set(c.x,c.y+c.lift+2.5,c.z);cameraDirection.copy(camera.position).sub(cameraAnchor);const distance=cameraDirection.length();cameraRay.set(cameraAnchor,cameraDirection.normalize());cameraRay.far=distance;
    const hit=cameraRay.intersectObjects(cameraBlockers,true).find(h=>h.distance>.4);
    if(hit){cameraClearance=hit.distance;camera.position.copy(cameraAnchor).addScaledVector(cameraDirection,Math.max(.35,hit.distance-.55));}
   }
   camera.lookAt(cameraPose.target.x,cameraPose.target.y,cameraPose.target.z);if(Math.abs(camera.fov-cameraPose.fov)>.05){camera.fov=cameraPose.fov;camera.updateProjectionMatrix();}
   if(shake&&game.phase==='racing')camera.rotateZ(-c.slip*(cameraMode==='first'?.035:.018));
   if(cockpitBody!==c.body){cockpitBody=c.body;const model=makeCockpit(colors[c.color],c.body);cockpit.children[0].geometry.dispose();cockpitGun.children[0].geometry.dispose();cockpit.children[0].geometry=model.body;cockpitGun.children[0].geometry=model.gun;cockpitGun.position.set(model.pivot.x,model.pivot.y,model.pivot.z);cockpitColor=-1;}
   if(cockpitColor!==c.color){cockpitColor=c.color;paintPart(cockpit.children[0],36,colors[c.color]);paintPart(cockpitGun.children[0],36,colors[c.color]);}
   cockpit.visible=cameraMode==='first';cockpitGun.rotation.y=c.aim;cockpitGun.rotation.x=c.pitch;cockpitGun.position.z=-.15+(c.cooldown>.95?(c.cooldown-.95)*.5:0);
  }else{
   const p=game.cars[previewSlot],orbit=Math.sin(renderTime*.16)*2;if(camera.fov!==55){camera.fov=55;camera.updateProjectionMatrix();}camera.position.set(p.x+Math.sin(p.heading+.75)*13+orbit,p.y+5.6,p.z+Math.cos(p.heading+.75)*13);camera.lookAt(p.x-Math.cos(p.heading+.75)*4.5,p.y+1.3,p.z+Math.sin(p.heading+.75)*4.5);cockpit.visible=false;
  }
  for(const c of game.cars){
   const m=tanks[c.id],opacity=started&&cameraMode==='third'&&c.id!==playerSlot?RacingCamera.occluderOpacity(game.cars[playerSlot],c,camera.position):1,transparent=opacity<.999;
   if(m.material.transparent!==transparent){m.material.transparent=transparent;m.material.depthWrite=!transparent;m.material.needsUpdate=true;}
  ...
  let count=0;for(const e of game.events){
   if(e.id>lastEvent){if(e.type==='driftboost'&&e.car===playerSlot){driftRewardTier=e.tier;driftRewardUntil=game.time+(R.DRIFT_TIERS[e.tier-1]?.boost||.8);}if(e.car===playerSlot||Math.hypot(e.x-game.cars[playerSlot].x,e.z-game.cars[playerSlot].z)<45)beep(e.type);lastEvent=e.id;}
   if(!['hit','fire','shield','pickup'].includes(e.type))continue;const age=.65-e.ttl;
   for(let k=0;k<(effectiveQuality==='low'?3:7)&&count<128;k++){const a=k*2.4+e.id,r=age*(e.type==='fire'?5:10);setInstance(effects,count++,e.x+Math.sin(a)*r,e.y+1.5+Math.sin(k+1)*r*.6,e.z+Math.cos(a)*r,Math.max(.05,e.ttl)*(e.type==='fire'?.6:1));effects.setColorAt(count-1,sparkColors[0]);}
  }
  for(const c of game.cars)if(c.drifting){for(let i=0;i<5&&count<128;i++){const a=renderTime*3+i;setInstance(effects,count++,c.x-Math.sin(c.heading)*(3+i*.7)+Math.cos(a)*1.3,c.y+.3+(i%3)*.2,c.z-Math.cos(c.heading)*(3+i*.7)+Math.sin(a)*1.3,.25+i*.08);effects.setColorAt(count-1,sparkColors[R.driftTier(c.driftCharge)]);}}
  for(const c of game.cars)if(c.boost>0)for(const side of [-1,1])for(let i=0;i<3&&count<128;i++){
   setInstance(effects,count++,c.x-Math.sin(c.heading)*(2.5+i*.55)+Math.cos(c.heading)*side*.95,c.y+.65,c.z-Math.cos(c.heading)*(2.5+i*.55)-Math.sin(c.heading)*side*.95,.45-i*.09);
   effects.setColorAt(count-1,sparkColors[c.id===playerSlot&&driftRewardUntil>game.time?driftRewardTier:1]);
  }
  effects.count=count;effects.instanceMatrix.needsUpdate=true;if(effects.instanceColor)effects.instanceColor.needsUpdate=true;
 ...
 function frame(now){
  ...
  if(!manual){accumulator+=elapsed;let steps=0;while(accumulator>=1/60&&steps<6){update(1/60);accumulator-=1/60;steps++;}}
   if(quality==='auto'&&started&&game.phase==='racing'&&!manual){qualityTime+=elapsed;frameSamples.push(elapsed*1000);if(frameSamples.length>180)frameSamples.shift();if(qualityTime>6&&frameSamples.length>=120&&frameSamples.reduce((a,b)=>a+b,0)/frameSamples.length>23&&NEXT_TIER[effectiveQuality]){setQuality(NEXT_TIER[effectiveQuality]);qualityTime=0;}}
  render();
 }
 window.render_game_to_text=()=>JSON.stringify({version:'10.1.0',circuit:{id:R.track.id,name:R.track.name,length:R.track.length,revision:R.track.revision},camera:{mode:cameraMode,eye:camera.position.toArray(),ownTankVisible:tanks[playerSlot].group.visible,cockpitVisible:cockpit.visible,pivot:cockpitGun.position.toArray(),clearance:cameraClearance,fadedRivals:tanks.map((m,id)=>({id,opacity:m.material.opacity})).filter(m=>m.opacity<.999)},trackDetails,garage:{body:chosenBody,color:chosenColor,models:tanks.map(t=>t.bodyIndex)},spectacle:spectacleState,coordinates:'x/z ground plane; +y up; heading 0 toward +z',network:{role:net.role,code:net.code,slot:playerSlot,members:net.members,rtt:net.rtt,match:net.match,status:net.status},started,phase:game.phase,time:game.time,countdown:game.countdown,player:{...game.cars[playerSlot]},rank:R.ranking(game).findIndex(c=>c.id===playerSlot)+1,cars:game.cars.map(c=>({id:c.id,body:c.body,color:c.color,x:c.x,z:c.z,lap:c.lap,progress:c.progress,finished:c.finished})),threats:game.threats,shots:game.shots.length,mines:game.mines.length,item:game.cars[playerSlot].item,quality:effectiveQuality,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,results:game.results});
 ...
 if(new URLSearchParams(location.search).get('autotest')==='1')window.RacingTest={net,probeRoads(){scene.updateMatrixWorld(true);const roads=scene.getObjectByName('Kenney city circuit')?.children.filter(m=>m.name.startsWith('Kenney road /'))||[],ray=new T.Raycaster(),samples=[];for(let s=0;s<R.track.length;s+=13){const p=R.sample(s);ray.set(new T.Vector3(p.x,p.y+2,p.z),new T.Vector3(0,-1,0));ray.far=4;const hit=ray.intersectObjects(roads)[0];samples.push({s,expected:p.y,actual:hit?.point.y??null});}return samples;},get game(){return game;},start:startRace,autodrive(seconds){manual=true;for(let i=0;i<seconds*60;i++)update(1/60,R.autopilot(game.cars[playerSlot],game));render();},realtime(auto=false){autodriveForTest=auto;manual=false;last=performance.now();},inspect:()=>({view:cameraMode,camera:{position:camera.position.toArray(),ownTankVisible:tanks[playerSlot].group.visible,cockpitVisible:cockpit.visible,pivot:cockpitGun.position.toArray(),clearance:cameraClearance,fadedRivals:tanks.map((m,id)=>({id,opacity:m.material.opacity})).filter(m=>m.opacity<.999)},modelBodies:tanks.map(t=>t.bodyIndex),calls:renderer.info.render.calls,triangles:renderer.info.render.triangles,memory:{...renderer.info.memory},ratio:renderer.getPixelRatio(),size:{width:renderer.domElement.width,height:renderer.domElement.height}})};
 ...
 window.racingReady=true;requestAnimationFrame(frame);const invitedRoom=new URLSearchParams(location.search).get('room');if(invitedRoom&&/^[A-HJ-NP-Z2-9]{6}$/.test(invitedRoom)){$('roomInput').value=invitedRoom;$('joinRoom').click();}
})();
